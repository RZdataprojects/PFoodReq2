...
class BAMnet(nn.Module):
    def __init__(self, vocab_size, vocab_embed_size, o_embed_size, \
        ...
        self.use_entity_name = use_entity_name
        ...
        self.ans_enc = AnsEncoder(o_embed_size, hidden_size, \
                        ...
                        use_entity_name=self.use_entity_name, \
                        use_cuda=use_cuda)
    ...
    def init_word_emb(self, init_word_embed):
        if init_word_embed is not None:
            dog_embedding = np.array([0.11008, -0.38781, -0.57615, -0.27714, 0.70521, 0.53994, -1.0786, 0.1088, -0.1539, 0.10645])
            elephant_embedding = np.array([-0.23618, -0.18959, 0.2912, -0.26124, 0.64728, 0.023738, -0.58402, -0.52287, 0.083624, -0.085531])
            computer_embedding = np.array([-0.10767, 0.11053, 0.59812, -0.54361, 0.67396, 0.10663, -0.73381, -0.21349, 0.19702, -1.4249])
            concatenated_embedding = np.concatenate((dog_embedding, elephant_embedding, computer_embedding))
            # Modify the shape of concatenated_embedding to match init_word_embed along dimension 1
            concatenated_embedding_resized = np.resize(concatenated_embedding, (1, init_word_embed.shape[1]))

            # Concatenate the resized concatenated_embedding with init_word_embed
            init_word_embed = np.concatenate((concatenated_embedding_resized, init_word_embed))
            dog_embedding = np.array([0.12208, -0.38781, -0.57615, -0.27714, 0.70521, 0.53994, -1.0786, 0.1088, -0.1539, 0.10645])
            elephant_embedding = np.array([-0.24618, -0.18959, 0.2912, -0.26124, 0.64728, 0.023738, -0.58402, -0.52287, 0.083624, -0.085531])
            computer_embedding = np.array([-0.10467, 0.11053, 0.59812, -0.54361, 0.67396, 0.10663, -0.73381, -0.21349, 0.19702, -1.4249])
            concatenated_embedding = np.concatenate((dog_embedding, elephant_embedding, computer_embedding))
            # Modify the shape of concatenated_embedding to match init_word_embed along dimension 1
            concatenated_embedding_resized = np.resize(concatenated_embedding, (1, init_word_embed.shape[1]))
            init_word_embed = np.concatenate((concatenated_embedding_resized, init_word_embed))
    ...
    def kb_aware_query_enc(self, memories, query_vec, query_lengths, ans_mask, ctx_mask=None):
        # Question encoder
        Q_r = self.que_enc(query_vec, query_lengths)[0]
        ...
        ans_comp_val, ans_comp_key = self.ans_enc(x_bow, x_bow_len, x_type_bow, x_types, x_type_bow_len, x_path_bow, x_paths, x_path_bow_len, x_ctx_ent, x_ctx_ent_marks, x_ctx_ent_len, x_ctx_ent_num)
    ...
    def forward(self, memories, queries, query_marks, query_lengths, query_words, ctx_mask=None):
        ...
        # question word vec
        self.qw_vec = torch.mean(self.qw_embed(query_words.cuda()), 1)
        # answer type vec
        x_types = memories[5]
        ...
        (ans_val, ans_key), (q_att, Q_r), query_mask = self.kb_aware_query_enc(memories, query_vec, query_lengths, ans_mask, ctx_mask=ctx_mask)
...
class AnsEncoder(nn.Module):
    ...
    def __init__(self, o_embed_size, hidden_size, num_ent_types, num_relations, vocab_size=None, \
                    ...
                    ans_enc_dropout=None, mark_emb_size=None, mark_emb=None, use_entity_name=False, use_cuda=True):
        ...
        if self.use_entity_name:
            self.linear_bow_key = nn.Linear(hidden_size, hidden_size, bias=False)
            self.linear_bow_val = nn.Linear(hidden_size, hidden_size, bias=False)
            self.lstm_enc_name = EncoderRNN(vocab_size, self.vocab_embed_size, hidden_size, \
    ...
    def forward(self, x_bow, x_bow_len, x_type_bow, x_types, x_type_bow_len, x_path_bow, x_paths, x_path_bow_len, x_ctx_ents, x_ctx_ent_marks, x_ctx_ent_len, x_ctx_ent_num):
        ans_bow, ans_type_bow, ans_types, ans_path_bow, ans_paths, ans_ctx_ent = self.enc_ans_features(x_bow, x_bow_len, x_type_bow, x_types, x_type_bow_len, x_path_bow, x_paths, x_path_bow_len, x_ctx_ents, x_ctx_ent_marks, x_ctx_ent_len, x_ctx_ent_num)
        ans_val, ans_key = self.enc_comp_kv(ans_bow, ans_type_bow, ans_types, ans_path_bow, ans_paths, ans_ctx_ent)
        return ans_val, ans_key

    def enc_comp_kv(self, ans_bow, ans_type_bow, ans_types, ans_path_bow, ans_paths, ans_ctx_ent):
        if self.use_entity_name:
            ans_bow_val = self.linear_bow_val(ans_bow)
            ans_bow_key = self.linear_bow_key(ans_bow)
            ans_comp_val = [ans_bow_val]
            ans_comp_key = [ans_bow_key]
        else:
            ans_comp_val = []
            ans_comp_key = []
        ...
        ans_comp_val.extend([ans_type_bow_val, ans_paths_val, ans_ctx_val])
        ans_comp_key.extend([ans_type_bow_key, ans_paths_key, ans_ctx_key])
        return ans_comp_val, ans_comp_key

    def enc_ans_features(self, x_bow, x_bow_len, x_type_bow, x_types, x_type_bow_len, x_path_bow, x_paths, x_path_bow_len, x_ctx_ents, x_ctx_ent_marks, x_ctx_ent_len, x_ctx_ent_num):
        '''
        x_types: answer type
        ...
        '''
        if self.use_entity_name:
            x_bow_emb = self.embed(x_bow.view(-1, x_bow.size(-1)))
            x_bow_emb = F.dropout(x_bow_emb, p=self.word_emb_dropout, training=self.training)
            ans_bow = (self.lstm_enc_name(x_bow_emb, x_bow_len.view(-1))[1]).view(x_bow.size(0), x_bow.size(1), -1)
        else:
            ans_bow = False
        ...
        if self.ans_enc_dropout:
            if self.use_entity_name:
                ans_bow = F.dropout(ans_bow, p=self.ans_enc_dropout, training=self.training)
        ...
        return ans_bow, ans_type_bow, None, ans_path_bow, ans_paths, ans_ctx_ent
...
class EncoderRNN(nn.Module):
    def __init__(self, vocab_size, embed_size, hidden_size, dropout=None, \
        bidirectional=False, rnn_type='lstm', use_cuda=True):
