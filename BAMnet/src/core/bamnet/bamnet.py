...
class BAMnetAgent(object):
    ...
    def __init__(self, opt, ctx_stops, vocab2id):
        ...
        opt['cuda'] = not opt['no_cuda'] | torch.cuda.is_available()
        if opt['cuda']:
            print('[ Using CUDA ]')
    ...
    def train_step(self, xs, ys, step, is_training=True):
        ...
        with torch.set_grad_enabled(is_training):
            ...
            ctx_mask = to_cuda(ctx_mask)
            queries = to_cuda(torch.LongTensor(xs[1]))
            query_words = to_cuda(torch.LongTensor(xs[2]))
            query_marks = to_cuda(torch.LongTensor(np.array(xs[5])))
            query_lengths = to_cuda(torch.LongTensor(xs[6]))
    ...
    def predict_step(self, xs, cand_labels, margin, verbose=False):
        ...
        with torch.set_grad_enabled(False):
            ...
            queries = to_cuda(torch.LongTensor(np.array(xs[1])), self.opt['cuda'])
            query_words = to_cuda(torch.LongTensor(np.array(xs[2])), self.opt['cuda'])
            query_marks = to_cuda(torch.LongTensor(np.array(xs[5])), self.opt['cuda'])
            query_lengths = to_cuda(torch.LongTensor(np.array(xs[6])), self.opt['cuda'])
